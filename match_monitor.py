...
class MatchLogHandler(PatternMatchingEventHandler):
    ...
    def process(self, event):
        ...
        # data = read_json('../Game/build/io/game_status.json')
        data = read_json('../Game/io/game_status.json')
    
        # game end
        if (data['game']['era'] == 3) & (data['game']['turn'] == 20):
            file_name = 'match_test-{}.json'.format(datetime.now().strftime("%d-%m-%Y-%H-%M-%S"))
...
if __name__ == '__main__':
    ...
    observer.schedule(MatchLogHandler(), path=args[0] if args else '.')
    observer.start()
